...
if __name__ == '__main__':
    ...
    if not opts.quiet:
        if opts.manual and opts.agent == None:
            ...
            if opts.agent == 'random': displayCallback = lambda state: display.displayValues(a, state, "CHINTAN CURRENT VALUES")
            if opts.agent == 'value': displayCallback = lambda state: display.displayValues(a, state, "CHINTAN CURRENT VALUES")
            if opts.agent == 'q': displayCallback = lambda state: display.displayQValues(a, state, "CHINTAN CURRENT Q-VALUES")
    ...
    if opts.episodes > 0:
        print
        print "CHINTAN RUNNING", opts.episodes, "EPISODES"
        print
    returns = 0
    ...
    if opts.agent == 'q' and not opts.manual:
        try:
            display.displayQValues(a, message = "CHINTAN Q-VALUES AFTER "+str(opts.episodes)+" EPISODES")
            display.pause()
            display.displayValues(a, message = "CHINTAN VALUES AFTER "+str(opts.episodes)+" EPISODES")
            display.pause()
        except KeyboardInterrupt:
            sys.exit(0)
